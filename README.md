...
- **NAMASTE and ICD-11 TM2 codes are empty placeholders**, not guesses. Those code
  lists are not publicly downloadable, and a fabricated code is the specific failure
  mode the ministry that owns the NAMASTE portal would catch. A test enforces that no
  unverified code can leave the system.
