...
**This file is the source of truth. Do not paraphrase or "improve" it.**
Everything in the other docs is our interpretation; this is theirs.

## Listing metadata (verified from sih.gov.in, 29 Aug 2026)
...
| Submitted ideas at scrape time | 0 / 500 |
| Portal URL | https://www.sih.gov.in/sih2026PS |

## Known gaps in the official text

1. **A table is missing.** Section 3.2 of the Expected Solution contains the literal
   placeholder `• Insert Table*3.2` — the portal never renders the table. It most
   likely specified the deployment/hardware profile or a module breakdown.
   **Action: check the portal again before submission in case they patch it.**
2. We do not have the modal's dataset link, YouTube link, or SPOC contact details.
   Pull these from the portal.

> Text below is reproduced as published. Only mojibake from the copy-paste was
> repaired (`â€”` → `—`, `â€“` → `–`, `â€™` → `'`). No wording was changed.
> Their own typos and grammatical slips are preserved.
...
### 3.2 [MISSING FROM PORTAL]

> `• Insert Table*3.2`
>
> The portal shows this placeholder and never renders the table. Content unknown.
